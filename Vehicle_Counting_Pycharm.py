...
blob_id = 1
frame_counter = 0
DETECTION_INTERVAL = 10 if di == None else di
MAX_CONSECUTIVE_TRACKING_FAILURES = 3 if mctf == None else mctf
detector = 'yolo' if detector == None else detector
tracker = 'kcf' if tracker == None else tracker
f_height, f_width, _ = frame.shape

# init video object and log file to record counting
if record:
    output_video = cv2.VideoWriter('./videos/output.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 30,
                                   (f_width, f_height))
...
# set counting line
clposition = 'bottom' if clposition == None else clposition
...
while True:
    k = cv2.waitKey(1)
    if iscam or cap.get(cv2.CAP_PROP_POS_FRAMES) + 1 < cap.get(cv2.CAP_PROP_FRAME_COUNT):
        _, frame = cap.read()
        ...
        if frame_counter >= DETECTION_INTERVAL:
            ...
            # print(boxes.class)
            c_boxes = np.array(boxes)
            p = np.array([0, 0])
            col_2 = 255
            col_3 = 0
            if c_boxes.shape[0] > 1:
                p = close_pairs(c_boxes, 200)
                print(p)
                col_2 = 0
                col_3 = 255
        ...
        for _id, blob in blobs.items():
            ...
            # if p[0] != 0:
            #     print ('her')
            if _id > vehicle_count:
                if _id - vehicle_count - 1 in p:
                    cv2.rectangle(frame, (x, y), (x + w, y + h), (0, col_2, col_3), 2)  # red
                    cv2.putText(frame, 'v_' + str(_id), (x, y - 2), cv2.FONT_HERSHEY_DUPLEX, 1, (0, 255, 0), 2,
                                cv2.LINE_AA)
                else:
                    cv2.rectangle(frame, (x, y), (x + w, y + h), (0, col_3, col_2), 2)  # green
                    cv2.putText(frame, 'v_' + str(_id), (x, y - 2), cv2.FONT_HERSHEY_DUPLEX, 1, (0, 255, 0), 2,
                                cv2.LINE_AA)
            # print(x,y,w,h)
